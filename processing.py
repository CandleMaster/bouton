...
def extract_to_doc(f,url):
    ...
    # Only relavant to Danni and perhaps Yuan 
    ...
    # Creating links in word doc
    for key, value in extracted_links.items():
        p = doc.add_paragraph()
        p.paragraph_format.line_spacing=Pt(24)
        playlist_url=url

        if playlist_url=="": # Case 1, if there's no url input
            for key, value in extracted_links.items():
                hyperlink=add_hyperlink(p,value, key,'3755DF', False)
                p.add_run().add_break()
        else:
            try:
                playlist_number=int(playlist_url) # Case 2, if input is a number, map to exisitng translations
                ...
                for key, value in pre_tranlated_playlist.items():
                    if key in extracted_links: # Hyperlinking titles with links
                        ...
                        p.add_run().add_break()
                        # print("into try")
            except: # Case3, if there is a url
