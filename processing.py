...
def extract_to_doc(f,url):
    ...
    def add_links_to_docx(extracted,titles_translated,p_text):
        if p_text in extracted: # Hyperlinking titles with links
            hyperlink = add_hyperlink(p, extracted[p_text], titles_translated, '3755DF', False)
            p.add_run().add_break()
        else:   # Adding chapter headings
            p.add_run().add_break()
            heading=p.add_run(titles_translated)
            heading.font.bold=True
            p.add_run().add_break()
    
    # Translation option not available
        ...
        if "/playlist" in url:
            ...
            for playlist in playlist_titles_tagged:
                if playlist == playlist_titles_tagged[0]: # Remove the first heading
                    continue
                p_texts=playlist.text.strip()
                title_translated=p_texts
                add_links_to_docx(extracted_links,title_translated,p_texts)

        elif "/science-education" in url: # scape core sites
            playlist_titles_tagged=playlist_soup.find_all(class_=['lesson-title','head-content'])
            for playlist in playlist_titles_tagged:
                if playlist.h2 is not None:
                    p_texts=playlist.contents[0].strip()+" "+ playlist.h2.text.strip()
                else:
                    p_texts=playlist.text.replace("Free SampleFree access at this time","").strip()
                title_translated=p_texts
                add_links_to_docx(extracted_links,title_translated,p_texts)

        elif "/education" in url: # scrape SE sites
            playlist_titles_tagged=playlist_soup.find_all(class_=['catalog-video-info','catalog-section-title'])
            for playlist in playlist_titles_tagged:
                if playlist.h2 is not None:
                    p_texts=playlist.h2.text.strip()
                else:
                    p_texts=playlist.h1.text.strip()
                title_translated=p_texts
                add_links_to_docx(extracted_links,title_translated,p_texts)

    # Only relavant to Danni and Yuan 
    frc_success={} #2
    ...
    for name in file_names:
        with zipfile_ob.open(name) as fp:
            ...
            vid_name_clean=re.sub(pattern,'',vid_name).replace("- Concept"," - Concept").replace(" - Student Protocol","")
            extracted_links[vid_name_clean]=vid_link
    
    # Preparing paragraphs in word doc
    for key, value in extracted_links.items():
        p = doc.add_paragraph()
        p.paragraph_format.line_spacing=Pt(24)
        playlist_url=url

        # Case 1, if there's no url input
        if playlist_url=="": 
            for key, value in extracted_links.items():
                hyperlink=add_hyperlink(p,value, key,'3755DF', False)
                p.add_run().add_break()
        else:
            try:
                # Case 2, if input is a number, map to exisitng translations
