...
def extract_to_doc(f,url,translation_option):
    ...
    #     if f.path.endswith('.zip'): # Turns all imscc files into zip files
    # f_stem=Path(f).stem
            # f_zip=Path(f).rename(f'{f_stem}.zip')
    ...
    # with zipfile.ZipFile(f,'r') as zip_ref: # And unzips zip files
    #     zip_ref.extractall(f_stem)
    extracted_links = {}
    ...
    for key, value in extracted_links.items():
        ...
        try: # Mapping to exisitng translations
            ...
            # print("into except")
    # doc.add_paragraph(extracted_links)
        file_stream = io.BytesIO()
        doc.save(file_stream)
        file_stream.seek(0)
    # doc.save('templates/okay.docx')
